...
const PaymentManagementPage: React.FC = () => {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      if (!activeFilter) {
        setSummary({
          onHold: data.filter((b) => b.paymentStatus === PaymentStatus.PAID)
            .length,
          released: data.filter(
            (b) => b.paymentStatus === PaymentStatus.RELEASED,
          ).length,
          readyToRelease: data.filter(
            (b) => b.paymentStatus === PaymentStatus.READY_TO_RELEASE,
          ).length,
        });
      }
    } catch {
      ...
      <main className="flex-1 ml-64 p-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">
            View and control all held payments from confirmed bookings.
          </p>
        </div>
