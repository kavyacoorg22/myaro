...
const DisputePage: React.FC = () => {
  ...
  const columns = ["Dispute ID", "Booking ID", "Customer", "Beautician", "Action"];
  ...
  useEffect(() => {
    const fetchDisputes = async () => {
      ...
      setError(null);
      try {
        const res     = await adminApi.getAllDisputes({ page: currentPage, limit: LIMIT });
        const payload = res.data;
        setDisputes(Array.isArray(payload?.data?.data) ? payload.data.data : []);
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 ml-64 p-8">
        ...
        <h1 className="text-2xl font-semibold text-gray-800 mb-6">Active Disputes</h1>

        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="bg-gray-100 px-6 py-4 border-b border-gray-200">
            <h2 className="text-base font-semibold text-gray-700">Active dispute</h2>
          </div>
            ...
            <Table className="table-fixed w-full">
              <TableHeader>
                <TableRow className="border-b border-gray-100">
                  <TableHead className="text-xs font-semibold text-gray-500 uppercase tracking-wide px-4 py-3 w-[160px]">Dispute ID</TableHead>
                  ...
                  <TableHead className="text-xs font-semibold text-gray-500 uppercase tracking-wide px-4 py-3 w-[110px]">Beautician</TableHead>
                  ...
                  disputes.map((dispute, index) => (
                    ...
                    >
                      <TableCell className="px-4 py-4">
                        <TruncatedId id={dispute.disputeId ?? "—"} />
                      </TableCell>
                      <TableCell className="px-4 py-4">
                        <TruncatedId id={dispute.bookingId} />
                      </TableCell>
                      <TableCell className="px-4 py-4 text-sm text-gray-700">{dispute.customerName}</TableCell>
                      <TableCell className="px-4 py-4 text-sm text-gray-700">{dispute.beauticianName}</TableCell>
                      <TableCell className="px-4 py-4">
                        <button
                          onClick={() => setSelectedBookingId(dispute.bookingId)}
