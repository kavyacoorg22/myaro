...
const BeauticianBookingsPage: React.FC = () => {
  ...
  const fetchBookings = useCallback(async () => {
    ...
    try {
      ...
      const result = await BookingApi.getBeauticianBookings(status, page, LIMIT);
      console.log(result.data.data?.bookings+'resultttttttttttttttttttttt')
      if (result) {
        setItems(Array.isArray(result.data.data?.bookings) ? result.data.data?.bookings : []);
  ...
  const renderList = (variant: "completed" | "cancelled") => {
    const sectionTitle = variant === "completed" ? "Completed booking" : "Canceled booking";
    return (
      <div className="space-y-3">
        <h3 className="text-base font-semibold text-gray-800">{sectionTitle}</h3>
        {items.length === 0 ? (
    ...
    <div className="flex h-screen">
      ...
      <div className="flex-1 bg-gray-50 overflow-y-auto">
        ...
        <div className="sticky top-0 z-10 bg-gray-50 border-b border-gray-200 pl-72 pr-8 pt-6 pb-0">
          <h1 className="text-2xl font-bold text-gray-900">Home Service Bookings</h1>
          <p className="text-sm text-gray-500 mt-0.5">Manage your appointments</p>
...
const Section: React.FC<{ title: string; items: IBookingListItem[] }> = ({ title, items }) => (
  <div>
...
const EmptyState: React.FC<{ variant: "completed" | "cancelled" }> = ({ variant }) => (
