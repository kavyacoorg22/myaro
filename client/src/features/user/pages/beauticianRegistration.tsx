...
export default function BeauticianRegistration() {
  ...
  const [prefillData, setPrefillData] = useState<IBeauticianReRegistrationPrefillDto | null>(null); // ✅ ADDED

  // ✅ ADDED — fetch prefill only when coming back after rejection
  useEffect(() => {
    if (location.state?.isReRegistration) {
      BeauticianApi.reRegiterPrefil()
        .then(res => setPrefillData(res.data?.data ?? null))
        .catch(() => {});
    }
  }, []);
  ...
  const handleStep2Complete = async (
    ...
    try {
      ...
      await BeauticianApi.beauticianRegister(formData);
