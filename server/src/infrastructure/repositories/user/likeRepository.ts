...
export class LikeRepository extends GenericRepository<Like,LikeDoc> implements ILikeRepository{

  constructor(){
    super(likeModal)
  }

   async create(data: Omit<Like, "id" | "createdAt" | "updatedAt">): Promise<Like> {
     const doc=await likeModal.create(data)
     return this.map(doc)
   }

async findByUserIDAndPostId(userID:string,postId:string): Promise<Like | null> {
  const doc = await likeModal.findOne({ userId: new Types.ObjectId(userID), postId: new Types.ObjectId(postId) });
