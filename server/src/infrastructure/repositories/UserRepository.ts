...
export class MongoUserRepository
  extends GenericRepository<User, UserDoc>
  implements IUserRepository, IToggleActiveStatusRepository
{
  constructor() {
    super(UserModel);
  }
  ...
  async updateByUserId(userId: string, data: Partial<User>): Promise<boolean> {
    ...
    const result = await UserModel.updateOne({ _id: userOid }, { $set: data });
  ...
  async updateRoleAndVerification(
    userId: string,
    role: UserRole,
    isVerified: boolean,
  ): Promise<User | null> {
    ...
    const raw = await UserModel.aggregate([
      ...
      // ── 3. pivot: push each role count into a per-month doc ──
      {
        $group: {
          ...
          customers: {
            $sum: {
              $cond: [{ $eq: ["$_id.role", UserRole.CUSTOMER] }, "$count", 0],
            },
          },
          beauticians: {
            $sum: {
              $cond: [{ $eq: ["$_id.role", UserRole.BEAUTICIAN] }, "$count", 0],
            },
          },
        },
      },
