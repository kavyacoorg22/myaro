...
export class PostRepository
  extends GenericRepository<Post, PostDoc>
  implements IPostRepository
{
  ...
  async findMixedFeedPosts(
    ...
    return {
      posts,
      nextCursorTips: hasMoreTips
        ? slicedTips[slicedTips.length - 1].id.toString()
        : null,
      nextCursorRent: hasMoreRent
        ? slicedRent[slicedRent.length - 1].id.toString()
        : null,
    };
  }
  ...
  async findByBeauticianIds(
    ...
    const filter: FilterQuery<PostDoc> = {
      beauticianId: { $in: beauticianIds.map((id) => new Types.ObjectId(id)) },
    };
  ...
  async searchByLocation(
    ...
    const filter: FilterQuery<PostDoc> = {
      "location.formattedString": regex,
    };
    if (cursor) filter._id = { $lt: new Types.ObjectId(cursor) };
