...
async execute(beauticianId: string): Promise<IGetBeauticianDashboardOutPut> {
  ...
  const [stats, weeklyChart, monthlyChart, payoutSummary, totalEarnings, recentPayouts,  { avgRating, totalReviews }] =
    await Promise.all([
      ...
      this._payoutRepo.getEarningsSummary(beauticianId, beautician?.createdAt ?? new Date()),
