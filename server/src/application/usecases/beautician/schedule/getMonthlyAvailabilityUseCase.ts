...
export class GetMonthlyAvailabilityUseCase implements IGetMonthlyAvailabilityUSeCase {
  constructor(private _getAvailabilityUseCase: IGetAvailbilityUseCase) {}
  async execute(
    beauticianId: string,
    month: number,
    year: number,
  ): Promise<IGetmonthlyAvailabilityOutput> {
    ...
    await Promise.all(
      Array.from({ length: daysInMonth }, async (_, i) => {
        ...
        const { availability } = await this._getAvailabilityUseCase.execute(
          beauticianId,
          date,
        );
        ...
        if (availability.type === ScheduleType.LEAVE) {
          dates.push(
            toGetMonthlyAvailabilityDto(availability.date, ScheduleType.LEAVE),
          );
        } else if (availability.slots.length > 0) {
          dates.push(
            toGetMonthlyAvailabilityDto(
              availability.date,
