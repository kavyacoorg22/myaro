...
import {
  IBookingListItem,
  IGetBeauticianBookingsInput,
  IGetBeauticianBookingsResult,
...
export class GetBeauticianBookingsUSeCase implements IGetBeauticianBookingsUseCase {
  ...
  async execute({
    ...
    const userIds = [
      ...new Set<string>(bookings.map((b: Booking) => b.userId)),
    ];
    ...
    const enriched = bookings
      ...
      .filter(
        (b: IBookingListItem | null): b is IBookingListItem => b !== null,
      );
