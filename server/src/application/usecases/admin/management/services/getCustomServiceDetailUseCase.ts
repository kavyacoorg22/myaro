...
import { IGetCustomServiceDetailsUseCase } from "../../../../interface/admin/management/services/IGetCustomServiceDetails";
...
export class GetCustomServiceDetailUseCase implements IGetCustomServiceDetailsUseCase{
  ...
  async execute(customServiceId: string): Promise<IGetCustomServiceDetailResponse> {
    const customService=await this._customserviceRepo.findById(customServiceId)
    if(!customService)
    {
      throw new AppError(generalMessages.ERROR.NOT_FOUND,HttpStatus.NOT_FOUND)
    }

    const beautician=await this._userRepo.findByUserId(customService.beauticianId)
    if(!beautician)
    {
      throw new AppError(generalMessages.ERROR.NOT_FOUND,HttpStatus.NOT_FOUND)
    }
