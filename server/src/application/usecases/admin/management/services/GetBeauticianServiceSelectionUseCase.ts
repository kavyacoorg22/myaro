...
import { IGetBeauticianServiceSelectionUseCase } from "../../../../interface/beauticianService/IGetBeauticianServiceSelectionUseCase";
import { IBeauticianServiceSelectionResponse } from "../../../../interfaceType/serviceType";
import {
  toCategoryServiceSelectionDto,
  toCustomServiceSelectionDto,
...
export class GetBeauticianServiceSelectionUseCase implements IGetBeauticianServiceSelectionUseCase {
  ...
  async execute(
    beauticianId: string,
  ): Promise<IBeauticianServiceSelectionResponse> {
    ...
    const beauticianServices =
      await this._beauticianServiceRepo.findByBeauticianId(beauticianId);

    const beauticianServiceMap = new Map<string, BeauticianService>();
    const customServicesBySystemCategory = new Map<
      string,
      ICustomServiceSelectionDto[]
    >();
    const customServicesByCustomCategory = new Map<
      string,
      ICustomServiceSelectionDto[]
    >();
    ...
    for (const bs of beauticianServices) {
      if (bs.serviceId) {
        ...
        if (bs.categoryId) {
          ...
          customServicesBySystemCategory
            .get(bs.categoryId)!
            .push(customServiceDto);
        } else {
          ...
          customServicesByCustomCategory
            .get(categoryName)!
            .push(customServiceDto);
        }
      }
    ...
    const systemCategoriesDto = categories.map((category) => {
      const systemServicesDto = systemServices
        ...
        .map((service) =>
          toServiceSelectionDto(service, beauticianServiceMap.get(service.id)),
        );

      // Get custom services for this system category
      const customServicesDto =
        customServicesBySystemCategory.get(category.id) || [];
    ...
    const customCategoriesDto = Array.from(
      customServicesByCustomCategory.entries(),
