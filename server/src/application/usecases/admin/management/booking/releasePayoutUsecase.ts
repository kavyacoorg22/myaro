...
export class ReleasePayoutUseCase implements IReleasePayoutUSeCase {
  ...
  async execute(input: IReleasePayoutInput): Promise<IReleasePayoutOutPut> {
    ...
    if (existingPayout?.status === PayoutStatus.COMPLETED) {
      throw new AppError(
        paymentMessages.ERROR.PAYOUT_ALREADY_COMPLETED,
        HttpStatus.CONFLICT,
      );
    }
    ...
    // ── 6. Resolve status ──────────────────────────────────────────────────
    const payoutStatus = this._statusResolver.resolvePayoutStatus(
      razorpayPayout.status,
    );

    // ── 7. Update payout ───────────────────────────────────────────────────
    const updatedPayout = await this._payoutRepo.updateStatus(
      payout.id,
      payoutStatus,
    );
    ...
    if (!updatedBooking) {
      throw new AppError(
        bookingMessages.ERROR.BOOKING_UPDATE_FAILED,
