...
export class GetAllDisputesUseCase implements IGetAllDisputesUseCase {
  ...
  async execute({
    page = 1,
    limit = 10,
  }: IGetAllDisputeInput): Promise<IGetAllDisputeOutPut> {
    const { bookings, total } = await this._bookingRepo.findDisputed({
      page,
      limit,
    });
    ...
    const beauticianIds = [
      ...new Set(bookings.map((b: Booking) => b.beauticianId)),
    ];
