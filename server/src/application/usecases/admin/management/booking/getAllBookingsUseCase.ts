...
import {
  IGetAllBookingsInput,
  IGetAllBookingOutPut,
...
export class GetAllBookingUseCase implements IGetAllBookingsUseCase {
  ...
  async execute({
    ...
    limit = 10,
    paymentStatus,
  }: IGetAllBookingsInput): Promise<IGetAllBookingOutPut> {
    const { payments, total } = await this._paymentRepo.findAll({
      page,
      limit,
    ...
    const data = payments
      .map((payment: Payment) => {
        ...
        const beautician = booking
          ? beauticianMap.get(booking.beauticianId)
          : null;
