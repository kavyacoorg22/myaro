...
export class GetAllRefundsUseCase implements IGetAllRefundsUseCase {
  ...
  async execute({
    ...
    limit = 10,
    status,
  }: IGetAllRefundInput): Promise<IGetAllRefundOutput> {
    const { refunds, total } = await this._refundRepo.findAll({
      page,
      limit,
      status,
    });
    ...
    const data = refunds
      .map((refund: Refund): IGetAllRefundsDto | null => {
        ...
        return toAdminRefundListItem(refund, booking, `${user.fullName}`);
      })
