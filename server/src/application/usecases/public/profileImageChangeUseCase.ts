...
async execute(
    ...
    try {
      ...
      throw new AppError("Failed to upload profile image", HttpStatus.INTERNAL_SERVER_ERROR);
    }

    
    const userData = await this._userRepo.updateProfileImageById(id, profilePath);
    
    
    if (!userData) {
      throw new AppError("Failed to update user profile", HttpStatus.INTERNAL_SERVER_ERROR);
    }
